...
- **Narration** — the executor's own progress logging: lifecycle events emitted as they happen, on the executor's own stream, never the payload's.
...
library-abandonment event. So a default *workload* budget exists only to
protect the executor, is set at machine scale, and is scoped to the
per-run aggregate — never a task-scale guess, never a per-item proxy for
the resource actually being protected. Machine-protection budgets are
per-run and do not compose: protecting the machine from N concurrent runs
is a separately declared batch- or fleet-level budget, never an implicit
multiplication. Executor self-budgets, which bound the machinery's own
small operations, are exempt from machine-scale sizing. A budget kill is
never the artifact of an unnoticed default. Preferring delivery modes that make permissiveness cheap (spooled
...
- Narration and record volume — executor self-budgets, accounted
  separately from payload output; exhaustion degrades narration, never
  the run.
...
- Every run leaves a durable record — what was invoked (argv, cwd,
  ...
  regardless of outcome: success never deletes the record. The record is
  guaranteed *attempted*: a record-write failure is itself recorded and
  attributed to the executor, and never fails the run.
...
- The record is faithful — the executor never *silently* edits, filters,
  or transforms what it observed: budget-driven truncation is always
  marked in the record with what was dropped and why, and nothing else is
  altered. Sensitivity is domain knowledge: redaction
...
- Narration and payload output are separate streams, never conflated —
...
remote-execution mode. Execution is also process-level: in-process
evaluation of payload content (`compile()` checks, importing checked-in
scripts) is outside the executor's scope and catalogued only as adjacent.
And everything the executor spawns is spawned from an argument vector —
nothing, in any use case, is ever interpreted by a shell.

1. **Untrusted Python source, call-scoped** — run generated Python in a budgeted, disposable runtime.
   - Fresh state per run by default — nothing a prior run did can affect this one; state is ever shared only across a declared sharing boundary (see the batch use case).
...
3. **Untrusted batch** — work structured as a cross-product of dimensions (candidates × tasks × cases), amortized through shared children at declared dimension boundaries, with results indexed by the dimensions.
   ...
   - The response echoes the request — the driver returns the identity of what it was asked (items, config), verified before any result is trusted. (Design requirement; not yet evidenced by an in-scope implementation.)
...
4. **Trusted tool invocation** — hardened calls to known programs (git, uv, linters, docker) with the lifecycle rigor ad-hoc call sites never have.
   ...
   - Argv only — commands are argument vectors; nothing is ever interpreted by a shell.
   - No survivors — when a call-scoped run ends, by any path, the child's entire process tree is gone before the call returns; supervised children get the same guarantee through deliberate shutdown (use case 6).
5. **Containment profiles** — real containment (filesystem/network/resource) for untrusted execution; mechanism to be researched (not necessarily docker); replaces and decommissions dr-docker.
...
6. **Supervised long-lived processes** — children that outlive the call (agent servers, stdio RPC); a fully targeted use case with its own distinct set of concerns (liveness, restart, streaming I/O, reaping across calls), designed as its own contract — never a mode of the call-scoped runners.
   ...
   - External deadlines are anticipated — when the environment will kill the run at a known time, shutdown begins with enough headroom that teardown and the final record are ours, not the environment's. (Design requirement; evidenced only partially, by dr_exp's lifetime cap.)
   ...
   - Supervision is commandable from outside — stop and drain can be requested by another local process, not only the spawner.
   ...
   - Interaction cancellation is first-class — in-flight work can be cancelled through the child's protocol without killing the child; signals address the process, cancellation addresses the request. Interaction-budget overflow triggers protocol cancellation under an executor self-budget, whose exhaustion escalates to process-level termination attributed to the channel.
...
7. **Supervised worker fleets** — many supervised children as one first-class unit: a slot map over resources, collective lifecycle, aggregate observability. Fleet is to supervised what batch is to call-scoped; experiment orchestration (job queues, sweeps, submission) consumes this contract and is never part of it.
   ...
   - Collective verbs are single operations — spawn, drain, and stop apply to the fleet as one call with per-slot results; slot failures are data, fleet-level faults raise, and the caller never loops over children.
   ...
   - Slots have declared lifecycle states — filled, retired, or failed; wind-down retires slots visibly, and an intentionally empty slot is never confusable with a dead one.
